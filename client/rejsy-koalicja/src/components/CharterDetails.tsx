...
export default function CharterDetails({charter}: CharterDetailsProps) {
    ...
    return (
        ...
        >
            ...
            <p><b>Total cost:</b> {charter.payoff?.totalCost}</p>
            <p><b>Total discount:</b> {charter.payoff?.discountValue || 0}%</p>
            <p><b>Voucher code:</b> {charter.payoff?.voucher?.code}
                {charter.payoff?.voucher?.code ? <React.Fragment> | discount: {charter.payoff?.voucher?.amount || 0}%</React.Fragment> : null}
            </p>
            <p><b>Discounts:</b>
            ...
            <p><b>Payments:</b>
                {charter.payoff?.payments?.map(function (payment, i) {
                    return (
                        <div>
                            <p>{i+1}. Payment date {formatDate(payment.paymentDate)} | Amount: {payment.amount}</p>
                        </div>
                    )
                })}
            </p>
